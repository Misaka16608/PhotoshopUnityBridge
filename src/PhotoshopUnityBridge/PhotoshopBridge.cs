...
/// blocking the HTTP request pipeline and to enable timeout support.
/// </summary>
public sealed class PhotoshopBridge : IDisposable
{
    ...
    private void InitializeCom()
    {
        try
        {
            ...
            _logger.LogInformation("Photoshop COM bridge initialized, version: {Version}",
                (string)_app.Version);
        }
        catch (Exception ex)
        {
    ...
    private void EnsureHealthy()
    {
        ...
        {
            throw new InvalidOperationException(
                "COM connection is unhealthy (previous call timed out). Restart the bridge server.");
        }
    }
